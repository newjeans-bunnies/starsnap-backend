...
class UserReportRepositoryImpl(
    ...
    override fun findSliceBy(pageable: PageRequest): Slice<UserReport>? {
        return userReportCrudRepository.findSliceBy(pageable.toSpringPageable())?.toCommonSlice()?.map { it.toDomain() }
    }

    override fun findSliceBy(pageable: PageRequest, userEntity: User): Slice<UserReport>? {
        return userReportCrudRepository.findSliceBy(pageable.toSpringPageable(), UserEntity.fromDomain(userEntity))?.toCommonSlice()?.map { it.toDomain() }
    }

    override fun findByIdOrNull(id: String): UserReport? {
        return userReportCrudRepository.findByIdOrNull(id)?.toDomain()
    }
    ...
    override fun save(userReport: UserReport): UserReport {
        return userReportCrudRepository.save(UserReportEntity.fromDomain(userReport)).toDomain()
    }
}