...
class FollowRepositoryImpl(
    ...
    override fun getFollowing(pageable: PageRequest, user: User): Slice<Follow>? {
        return followCrudRepository.getFollowing(pageable.toSpringPageable(), user)?.toCommonSlice()?.map { it.toDomain() }
    }

    override fun getFollowers(pageable: PageRequest, user: User): Slice<Follow>? {
        return followCrudRepository.getFollowers(pageable.toSpringPageable(), user)?.toCommonSlice()?.map { it.toDomain() }
    }

    override fun save(follow: Follow): Follow {
        return followCrudRepository.save(FollowEntity.fromDomain(follow)).toDomain()
    }
    ...
    override fun findByIdOrNull(id: String): Follow? {
        return followCrudRepository.findByIdOrNull(id)?.toDomain()
    }

    override fun findByFollowUserAndFollowerUserId(followUser: User, followerUserId: String): Follow? {
        return followCrudRepository.findByFollowUserIdAndFollowerUserId(followUser, followerUserId)?.toDomain()
    }
}